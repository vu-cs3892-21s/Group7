...
def get_github_user_info() -> UserRecord:
    ...
    new_user: User = User(name=user["name"], primary_email=primary_email)
...
def blueprint_logged_in(blueprint, token) -> bool:
    ...
    github_info: UserRecord = get_github_user_info()
    github_email: str = github_info["primary_email"]

    # Find this OAuth token in the database, or create it
    query = OAuth.query.filter_by(
        provider=blueprint.name,
        provider_user_email=github_email,
    )
    try:
        oauth = query.one()
    ...
    if oauth.user:
        ...
        user = User(
            primary_email=github_info["primary_email"],
...
# def google_sesssion():
#     return create_session(google, "google.login")
