...
class OAuth(OAuthConsumerMixin, db.Model):
    __table_args__ = (db.UniqueConstraint("provider", "provider_user_email"),)
    provider_user_email = db.Column(db.String(256), nullable=False)
    user_id = db.Column(db.String(40), db.ForeignKey(
        User.primary_email), nullable=False)
